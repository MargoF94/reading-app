...
| AO3 EPUB | Upload AO3's Download → EPUB file (fic form or Import page). The EPUB is unzipped in the browser and its AO3 preface page is read with the same parser; if that page is missing, the EPUB's own metadata (title, author, language, tags, summary, work link) is used. |
